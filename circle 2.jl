...
function plot_wrinkles_with_eigenvalues(sol, xp, yp, tol, d)
    # Define the function for calculating wrinkle directions and smallest eigenvalue
    f(xy) = cm.Point2f(get_wrinkles(xy, sol, tol))
    smallest_eigenvalue(xy) = begin
        μ = sol[4]
        x = xy[1]
        y = xy[2]
        if 0.25 - x^2 - y^2 < 0
            return NaN  # Outside the circular domain
        else
            μ_val = [μ[1, 1](x, y) μ[1, 2](x, y); μ[2, 1](x, y) μ[2, 2](x, y)]
            ev = eigen(μ_val)
    ...
    fig = cm.Figure(resolution=(500,500 ))
    ax = cm.Axis(fig[1, 1], title="Tolerance = $tol and Degree = $d", xlabel="x", ylabel="y")
    ...
    # Generate a grid of points to evaluate eigenvalues
    eigenvalues = [smallest_eigenvalue((x, y)) for x in xp, y in yp]
    
    # Plot the heatmap of smallest eigenvalues
    cm.heatmap!(ax, xp, yp, eigenvalues, colormap=:coolwarm, transparency=true)
    

    # Add the wrinkle streamplot
    cm.streamplot!(ax, f, xp, yp, arrow_size=0, stepsize=5e-3, maxsteps=500,colormap=:blues)
...
plt = plot_wrinkles_with_eigenvalues(sol, xp, yp, tol, d)
plt[1] # show plot
