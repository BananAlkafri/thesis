...
function plot_wrinkles(sol, xp, yp, tol, d)
    ...
    # Create a figure with default resolution
    fig = cm.Figure()
    
    # Create an axis with specific width and height (e.g., 400x400 for a smaller plot)
    ax = cm.Axis(fig[1, 1], width = 500, height = 350, title = "Tolerance = $tol and Degree = $d", xlabel = "x", ylabel = "y")
